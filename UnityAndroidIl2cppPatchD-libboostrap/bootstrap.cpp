...
JNIEXPORT void JNICALL Java_io_github_noodle1983_Boostrap_init(JNIEnv * jenv, jclass cls, jstring path)
{
...
__attribute__ ((visibility ("default")))
JNIEXPORT void JNICALL Java_io_github_noodle1983_Boostrap_hook(JNIEnv * jenv, jclass cls)
{
	bootstrap();
}
...
__attribute__ ((visibility ("default")))//old:char* get_arch_abi()
JNIEXPORT jstring JNICALL Java_io_github_noodle1983_Boostrap_getarchabi(JNIEnv * jenv, jclass cls)
{
	char* abi = dupstr(TARGET_ARCH_ABI);
	return jenv->NewStringUTF(abi);
}

char* g_use_data_path = NULL;
__attribute__ ((visibility ("default")))//old：char* use_data_dir(const char* data_path, const char* useless)
JNIEXPORT jstring JNICALL Java_io_github_noodle1983_Boostrap_usedatadir(JNIEnv * jenv, jclass cls, jstring data_path, jstring package_name)
{	
    const char* data_file_path_temp = jenv->GetStringUTFChars(data_path, NULL); 
	char* data_file_path = dupstr(data_file_path_temp);
	jenv->ReleaseStringUTFChars(data_path, data_file_path_temp);
	
	const char* package_name_temp = jenv->GetStringUTFChars(package_name, NULL); 
	char* package_name_path = dupstr(package_name_temp);
	jenv->ReleaseStringUTFChars(package_name, package_name_temp);
	...
	if (strlen(data_file_path) > 0){
		DIR* dir = opendir(data_file_path);
		if (dir == NULL)
		{
			MY_ERROR("can't access data path:%s", data_file_path);
	        return jenv->NewStringUTF(strdup("can't access data dir!"));			
		}
		closedir(dir);
	}
	
	//std::string bundle_id = get_bundle_id();
	std::string bundle_id = package_name_path;
    //1. prepare so lib
    prepare_so_lib(data_file_path, bundle_id);
	
    //2. pre-find apk path  
	std::string apk_path_string = get_apk_path(bundle_id);
    //3. pre-calc partition
    ShadowZipGlobalData global_data;
    if(0 != ShadowZip::init(data_file_path, apk_path_string.c_str(), &global_data)){
		char error_str[256] = {0};
		snprintf(error_str, sizeof(error_str), "failed to do ShadowZip::init(%s, %s)", data_file_path, apk_path_string.c_str());
		MY_ERROR("%s", error_str);
		return jenv->NewStringUTF(dupstr(error_str));			
    }
    ...
    serial_string(fw, data_file_path);
